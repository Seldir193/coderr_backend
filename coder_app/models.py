...
class Review(models.Model):
    ...
    offer = models.ForeignKey(
        Offer, on_delete=models.CASCADE, related_name='offer_reviews', null=True, blank=True
    ) 
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Order(models.Model):
    ...
    def save(self, *args, **kwargs):
        """
        Überschreibt die save-Methode, um `business_user` und `features` zu setzen.
        """
        if not self.business_user and self.offer:
            self.business_user = self.offer.user

        if self.offer_detail and not self.features:
            self.features = self.offer_detail.features or []

        if not self.title:
            self.title = self.offer.title

        if not self.price:
            self.price = self.offer_detail.variant_price

        if not self.delivery_time_in_days:
