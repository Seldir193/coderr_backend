...
class OfferDetailAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.offer = Offer.objects.create(
            title="Test Offer", description="This is a test offer.", price=99.99
        )
    ...
    def test_offer_detail_change_view(self):
        ...
        response = self.client.get(
            reverse("admin:coder_app_offerdetail_change", args=[self.offer_detail.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Variant"
        )  # Überprüfe, ob der Variant Titel im Formular angezeigt wird
    ...
    def test_offer_detail_list_view(self):
        ...
        self.assertContains(
            response, "Test Variant"
        )  # Überprüfe, ob der Variant Titel in der Liste angezeigt wird

    def test_offer_detail_search(self):
        # Teste die Suchfunktion für das Angebot Detail
        response = self.client.get(
            reverse("admin:coder_app_offerdetail_changelist") + "?q=Test Variant"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Variant"
        )  # Überprüfe, ob der Variant Titel in den Suchergebnissen angezeigt wird

    def test_offer_detail_filter(self):
        # Teste die Filterfunktion für das Angebot Detail
        response = self.client.get(
            reverse("admin:coder_app_offerdetail_changelist") + "?offer_type=Standard"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Variant"
        )  # Überprüfe, ob der Variant Titel in den gefilterten Ergebnissen angezeigt wird
...
class ReviewAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.reviewer = User.objects.create_user(
            username="reviewer", password="reviewerpass", email="reviewer@example.com"
        )
        self.business_user = User.objects.create_user(
            username="business", password="businesspass", email="business@example.com"
        )
        self.offer = Offer.objects.create(
            title="Test Offer", description="This is a test offer.", price=99.99
        )  # Beispielangebot
    ...
    def test_review_change_view(self):
        ...
        response = self.client.get(
            reverse("admin:coder_app_review_change", args=[self.review.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Great service!"
        )  # Überprüfe, ob die Beschreibung im Formular angezeigt wird
    ...
    def test_review_list_view(self):
        ...
        self.assertContains(
            response, "reviewer"
        )  # Überprüfe, ob die Beschreibung in der Liste angezeigt wird

    def test_review_filter(self):
        # Teste die Filterfunktion für die Bewertung
        response = self.client.get(
            reverse("admin:coder_app_review_changelist") + "?rating=5"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "reviewer"
        )  # Überprüfe, ob der Name des Reviewers in den gefilterten Ergebnissen angezeigt wird

    def test_review_search(self):
        # Teste die Suchfunktion für die Bewertung
        response = self.client.get(
            reverse("admin:coder_app_review_changelist") + "?q=reviewer"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "reviewer"
        )  # Überprüfe, ob der Name des Reviewers in den Suchergebnissen angezeigt wird
...
class OrderAdminTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="customer", password="customerpass", email="customer@example.com"
        )
        self.business_user = User.objects.create_user(
            username="business", password="businesspass", email="business@example.com"
        )
    ...
    def test_order_change_view(self):
        ...
        response = self.client.get(
            reverse("admin:coder_app_order_change", args=[self.order.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob das Angebot im Formular angezeigt wird
    ...
    def test_order_list_view(self):
        ...
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob das Angebot in der Liste angezeigt wird

    def test_order_search(self):
        # Teste die Suchfunktion für die Bestellung
        response = self.client.get(
            reverse("admin:coder_app_order_changelist") + "?q=customer"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob die Bestellung in den Suchergebnissen angezeigt wird

    def test_order_filter(self):
        # Teste die Filterfunktion für die Bestellung
        response = self.client.get(
            reverse("admin:coder_app_order_changelist") + "?status=Pending"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob die Bestellung in den gefilterten Ergebnissen angezeigt wird
...
class OfferAdminTest(TestCase):
    ...
    def test_offer_change_view(self):
        ...
        response = self.client.get(
            reverse("admin:coder_app_offer_change", args=[self.offer.id])
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob der Titel im Formular angezeigt wird
    ...
    def test_offer_list_view(self):
        ...
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob der Titel in der Liste angezeigt wird

    def test_offer_search(self):
        # Teste die Suchfunktion für das Angebot
        response = self.client.get(
            reverse("admin:coder_app_offer_changelist") + "?q=Test Offer"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob das Angebot in den Suchergebnissen angezeigt wird

    def test_offer_filter(self):
        # Teste die Filterfunktion für das Angebot
        response = self.client.get(
            reverse("admin:coder_app_offer_changelist") + "?price__exact=99.99"
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Offer"
        )  # Überprüfe, ob das Angebot in den gefilterten Ergebnissen angezeigt wird
...
class BusinessProfileAdminTest(TestCase):
    ...
    def test_business_profile_change_view(self):
        ...
        response = self.client.get(
            reverse(
                "admin:coder_app_businessprofile_change",
                args=[self.business_profile.id],
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Company"
        )  # Überprüfe, ob der Firmenname im Formular angezeigt wird
    ...
    def test_business_profile_list_view(self):
        ...
        response = self.client.get(
            reverse("admin:coder_app_businessprofile_changelist")
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Test Company"
        )  # Überprüfe, ob der Firmenname in der Liste angezeigt wird

    def test_profile_image_preview(self):
        # Teste die Vorschau des Profilbilds
        self.business_profile.profile_image = (
            "path/to/image.jpg"  # Setze einen Beispielpfad
        )
        ...
        )  # Überprüfe, ob der Bildpfad in der Antwort enthalten ist
...
class CustomerProfileAdminTest(TestCase):
    ...
    def test_customer_profile_change_view(self):
        ...
        response = self.client.get(
            reverse(
                "admin:coder_app_customerprofile_change",
                args=[self.customer_profile.id],
            )
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "John Doe"
        )  # Überprüfe, ob der Name im Formular angezeigt wird
    ...
    def test_customer_profile_list_view(self):
        ...
        response = self.client.get(
            reverse("admin:coder_app_customerprofile_changelist")
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "John Doe"
        )  # Überprüfe, ob der Name in der Liste angezeigt wird
...
class OrderModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.offer_detail = OfferDetail.objects.create(
            offer=self.offer, features=["Feature 1", "Feature 2"]
        )

    def test_order_creation(self):
        # Teste die Erstellung einer Bestellung
        order = Order.objects.create(
            user=self.customer_user, offer=self.offer, offer_detail_id=self.offer_detail
        )
        ...
        )  # Standardstatus sollte 'pending' sein
        self.assertEqual(
            order.business_user, self.business_user
        )  # business_user sollte gesetzt werden
    ...
    def test_set_features_from_offer_detail_if_missing(self):
        ...
        self.assertEqual(
            order.features, ["Feature 1", "Feature 2"]
        )  # Features sollten gesetzt werden
    ...
    def test_order_status_choices(self):
        ...
        self.assertIn(
            order.status, dict(Order.STATUS_CHOICES)
        )  # Status sollte in den Wahlmöglichkeiten sein
...
class ReviewModelTest(TestCase):
    ...
    def test_string_representation(self):
        ...
        self.assertEqual(
            str(review),
            f"Review by {self.reviewer} for (Business: {self.business_user}, Offer: {self.offer})",
        )
    ...
    def test_review_ordering(self):
        ...
        self.assertEqual(
            list(reviews), [review2, review1]
        )  # review2 sollte zuerst kommen
...
class OfferDetailModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.offer = Offer.objects.create(
            title="Test Offer", description="Test Description", price=100.00
        )
    ...
    def test_offer_type_choices(self):
        ...
        offer_detail = OfferDetail(
            offer=self.offer, variant_title="Test Variant", offer_type="standard"
        )
        self.assertIn(
            offer_detail.offer_type,
            dict(OfferDetail._meta.get_field("offer_type").choices),
        )
...
class CustomerProfileSerializerTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.customer_profile = CustomerProfile.objects.create(
            user=self.user, first_name="Test", last_name="User", created_at="2024-01-01"
        )
    ...
    def test_serializer_update(self):
        ...
        serializer = CustomerProfileSerializer(
            instance=self.customer_profile, data=data, partial=True
        )
        self.assertTrue(serializer.is_valid(), serializer.errors)
...
class OrderSerializerTests(APITestCase):
    ...
    def test_serializer_update(self):
        ...
        data = {"status": "in_progress", "offer_detail_id": self.offer_detail.id}
...
class OfferSerializerTests(TestCase):
    ...
    def test_serializer_update(self):
        ...
        serializer = OfferSerializer(
            instance=self.existing_offer, data=updated_data, partial=True
        )
        self.assertTrue(serializer.is_valid(), serializer.errors)
        ...
        self.assertEqual(
            updated_detail.features, ["Updated Feature 1", "Updated Feature 2"]
        )
...
class OfferDetailSerializerTests(TestCase):
    ...
    def test_partial_update(self):
        ...
        serializer = OfferDetailSerializer(
            instance=self.offer_detail, data=partial_data, partial=True
        )
...
class LoginSerializerTests(TestCase):
    ...
    def test_empty_username(self):
        ...
        """
        data = {"username": "", "password": "securepassword"}
        serializer = LoginSerializer(data=data)
        self.assertFalse(serializer.is_valid())
        self.assertIn("username", serializer.errors)
...
class RegistrationSerializerTests(TestCase):
    ...
    def test_duplicate_email(self):
        ...
        """
        User.objects.create(
            username="existinguser", email="duplicate@example.com", password="dummy"
        )
        data = {
            "username": "newuser",
